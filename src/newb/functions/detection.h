...
bool detectEnd(float DIMENSION_ID, vec3 FOG_COLOR) {
  vec3 endFogCol = vec3(0.043, 0.031, 0.047);
  bool fogMatch = length(FOG_COLOR - endFogCol) < 0.01;
  return (DIMENSION_ID == 2.0) || fogMatch;
}
...
bool detectNether(float DIMENSION_ID, vec3 FOG_COLOR, vec2 FOG_CONTROL) {
  ...
  bool netherFog = FOG_COLOR.b < 0.02 && FOG_COLOR.r > 0.1 && (FOG_COLOR.r-FOG_COLOR.g) > 0.05;
  return (DIMENSION_ID == 1.0) || underLava || netherFog;
}
...
nl_environment nlDetectEnvironment(float DIMENSION_ID, float TIME_OF_DAY, float DAY, vec3 FOG_COLOR, vec3 FOG_CONTROL) {
  nl_environment env;
  env.end = detectEnd(DIMENSION_ID, FOG_COLOR);
