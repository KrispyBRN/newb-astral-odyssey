...
float nlRenderGodRayIntensity(vec3 cPos, vec3 worldPos, float t, vec2 uv1, float relativeDist, vec3 FOG_COLOR, float moonPhase, vec3 moonDir) {
  ...
  float vol = 0.0;
    ...
    float volS = sin(7.0*uS + 1.5*diff)*sin(3.0*uS + diff);
    ...
    float maskM = rx*rx;
    float volM = sin(7.0*uM + 1.5*diff)*sin(3.0*uM + diff);
