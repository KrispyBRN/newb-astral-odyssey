  ...
  uniform vec4 FogAndDistanceControl;
    ...
    vec3 nlGetAurora3D(vec3 vDir, float time, float dither) {
      ...
      for (int i = 0; i < sampleCount; i++) {
        ...
        aurora += noise*currentM*mix(NL_AURORA_3D_COL_FAR, NL_AURORA_3D_COL_NEAR, nlPow2(nlPow2(currentM)));
      }
...
void main() {
    ...
    vec3 skyColor = nlRenderSky(skycol, env, -viewDir, v_underwaterRainTimeDay.z, true);
      ...
      if (aurora3DMask > 0.01) {
        float dither = fract(sin(dot(gl_FragCoord.xy, vec2(12.9898, 78.233)))*43758.5453);
        skyColor += nlGetAurora3D(viewDir, v_underwaterRainTimeDay.z, dither)*aurora3DMask;
      }
    #endif
