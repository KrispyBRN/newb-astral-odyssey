...
#include <bgfx_shader.sh>
  ...
  uniform vec4 FogAndDistanceControl;
    ...
    vec3 nlGetAuroraCurtain(vec3 vDir, float time) {
      ...
      for (int i = 0; i < 3; i++) {
        ...
        float warp = 2.0*noise2D(vec2(ang*1.3 + fi*4.1, elev*2.5 - t*0.4)) - 1.0;
...
void main() {
    ...
    vec3 skyColor = nlRenderSky(skycol, env, -viewDir, v_underwaterRainTimeDay.z, true);
      ...
      if (auroraMask > 0.01) {
        skyColor += NL_AURORA_CURTAIN*nlGetAuroraCurtain(viewDir, v_underwaterRainTimeDay.z)*auroraMask;
      }
    #endif
