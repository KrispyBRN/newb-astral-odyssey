...
void main() {
  v_texcoord0 = a_texcoord0;
    ...
    if (isSun) {
      ...
      float dawnDist = min(TimeOfDay.x, 1.0-TimeOfDay.x);
      float duskDist = abs(TimeOfDay.x - 0.5);
      float dawnFactor = 1.0 - smoothstep(0.0, NL_SUN_DAWN_DUSK_RANGE, dawnDist);
      float duskFactor = 1.0 - smoothstep(0.0, NL_SUN_DAWN_DUSK_RANGE, duskDist);
