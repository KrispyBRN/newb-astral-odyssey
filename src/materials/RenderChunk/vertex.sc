...
void main() {
  ...
  #if (defined(NL_GODRAY) || defined(NL_GODRAY_FULLMOON)) && defined(NL_FOG)
    vec2 godray = nlRenderGodRayIntensity(cPos, worldPos, t, uv1, relativeDist, FogColor.rgb, MoonPhase.x, env.moonDir);
    fogColor.rgb = mix(fogColor.rgb, NL_GODRAY_FULLMOON_COL, min(godray.y, 1.0));
    fogColor.a = mix(fogColor.a, 1.0, min(godray.x, 1.0));
  #endif
